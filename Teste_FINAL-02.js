...
function searchYouTube(authHeaders, query) {
...
export default function () {
  ...
  let results = searchYouTube(authHeaders, searchTerm);

  let attempts = 0;
  while (!results && attempts < 5 && vuId <= 53) {
    const fallbackQuery = QUERIES[Math.floor(Math.random() * QUERIES.length)];
    results = searchYouTube(authHeaders, fallbackQuery);
    attempts++;
  }
  ...
  if (results && results.length > 0) {
    ...
    if (vuId <= 53) {
      ...
      if (enqueueRes.status === 200) {
        ...
        console.log(`[VU ${vuId}] Erro enqueue: ${enqueueRes.status} - ${enqueueRes.body}`);
        errorRate.add(1);
      }
    }
  } else if (vuId <= 53) {
